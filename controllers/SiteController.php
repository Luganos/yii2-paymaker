...
//------------------------------------------------------------------------------------------------//
    ...
    public function actionLogin()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->login()) 
        {
            ...
            //Change status current user on online
            $customer->onlineCustomer($user);
    ...
    public function actionLogout()
    {
        ...
        //Change status current user on offline
        $customer->offlineCustomer($user);
...
//------------------------------------------------------------------------------------------------//
    ...
    {  
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate())
        {
            ...
            $newTable = $customer->createCustomerTable($name);
            $newCustomer = $customer->insertCustomer($user->username);
            // try to save user data in database
            if ($user && $newTable && $newCustomer) 
            {
                ...
                if ($user->status === User::STATUS_ACTIVE)
                {
                    if (Yii::$app->getUser()->login($user)) 
                    {
                        ...
                        //Change status current customer on online
                        $customer->onlineCustomer($user->username);
                        return $this->goHome();
                    }
                }
